...
{ 
  ...
  if (Servo_Channel == num_servos){           // This is the SYNC PULSE
    ...
    // Normal servo operation (50Hz). We need the maximun pulse length of 16.3ms. You can reduce this if you have more than 3 or 4 servos...
    // TCNT2 = 0;
    // 125 Hz rate (8ms total frame time)
    aux = 8000 - (ch3<<2);  // 8000us - 4x(channel pulse width) = SYNC pulse width
    aux = 0xFF - (aux>>6);  // 256(timer2 overflow) - (SYNC pulse width/64us) 
    TCNT2 = (byte)aux;
    //TCNT2 = 0xC0;
    }
  else{
