...
Check not mixing real with % thrust on PWM thrust vars
...
Need to set constrain on PWM value to doesn't dip below MINIMUM otherwise motors power off
...
//Set objects  --------------------------------------------------------------
  ...
  // ########## Altitude/Barometric Setup ------------------------
    ...
    long current_pascals = 101900;
        #define numHistory 5 // how many samples of moving-average to use
...
// Configure all pin names and outputs  
  ...
  //Start-up the Barometer circuitry
    // 0-3 levels set sensitivity of sensor, more sensitive is slower to respond...
    BMP085_Alt.begin(3);    
 ...
 // PID enable and configure ranges and modes
    ...
    // Altitude affects all motors equally from 0-100% thrust while yaw, pitch and roll are treated as 'incremental' 50% deltas rather than absolute values for thrust
    X_axis_PID.SetOutputLimits(0,50);
    Y_axis_PID.SetOutputLimits(0,50);
  ...
  // Attaches the thrust servos PWM out to specific Arduino pins
    ...
    // Wait 2 seconds for ESCs to register min input value
    delay(2000);
    ...
    // Wait 3 seconds for ESCs to spin up
    delay(3000);
...
//           Autopilot_status = 0;
  ...
  if(historyPtr == -1)
  {
    ...
    {
      history[i] = BMP085_Alt.readAltitude(current_pascals);
    }
    historyTotal = int(numHistory) * BMP085_Alt.readAltitude(current_pascals);
    historyPtr = 0;
    // Read ground height  
    ground_altitude = BMP085_Alt.readAltitude(current_pascals);
  ...
  else
  {
    ...
    history[historyPtr] = BMP085_Alt.readAltitude(current_pascals);
    historyTotal += BMP085_Alt.readAltitude(current_pascals);
    historyPtr++;
  ...
  int altitude_current = (historyTotal / int(numHistory))-ground_altitude;
 ...
 // Instruct thrust motors to alter output to new PWM values based on converting % output to 'real' PWM min/max range
   ...
   // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& need to constrain values first
