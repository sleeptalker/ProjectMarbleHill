...
// Configure all pin names and outputs  
 ...
 // PID enable and configure ranges and modes
    ...
    // Trying to not allow X or Y + Alt correction to >100%
    X_axis_PID.SetOutputLimits(0,5);
    Y_axis_PID.SetOutputLimits(0,5);
    Altitude_PID.SetOutputLimits(0,95);
  ...
  // Attaches the thrust servos PWM out to specific Arduino pins
    ...
    // Wait 2 seconds for ESCs to spin up
    delay(2000);
...
void loop() {
  ...
  // Read for RC input (ideally an interrupt) and convert to meaningful signal  
    ...
    rc_channel_2 = pulseIn(rc_channel_2_pin, HIGH, 25000); // each RC channel
        ...
        // Remap RC channel input to 0 to 85 (or 20 if non-altitude) range - representing % thrust
        rc_channel_1 = map(rc_channel_1,rc_channel_lower_limit,rc_channel_upper_limit,0,85);
