...
Current Testing Tweaks:
  ...
  Commented out part due to no true Yaw sensing
...
//Set objects  --------------------------------------------------------------
  ...
  // ########## PID Input/Output Tuning setup  ----------------
    ...
    // Aggressive
    double aggKp=2.3, aggKi=0.15, aggKd=0.001;
    // Conservative
    double consKp=2.3, consKi=0.15, consKd=0.001;
...
// Configure all pin names and outputs  
 ...
 // PID enable and configure ranges and modes
    ...
    X_axis_PID.SetOutputLimits(-1,1);
    Y_axis_PID.SetOutputLimits(-80,80);
    Z_axis_PID.SetOutputLimits(-1,1);
    Altitude_PID.SetOutputLimits(-10,10);
...
// To compensate for RC input noise adjust yaw,pitch,roll values to 0 if -8 -> +2 %
...
// If Autopilot RC input ON - then check and maintain stable horizon and altitude - at minimum 'safe' altitude or last recorded 'current' altitude
// To prevent RC noise the auto pilot value must be at least 80% ON
if (rc_channel_6 >= 80){     
  
     // Enable AUTOPILOT
       // PID - Change all axial PID control to 'AUTOMATIC' to allow full autopilot control
       X_axis_PID.SetMode(AUTOMATIC);
       Y_axis_PID.SetMode(AUTOMATIC);
       Z_axis_PID.SetMode(AUTOMATIC);
       Altitude_PID.SetMode(AUTOMATIC);
...
InputAltitude = altitude_current; 
  ...
  // ***************************
  // ***************************
  // Forced to 0 until YAW and Altutude sensors are sorted
...
// Adjustment is based on using thrust needed to sustain altitude and +/- % thrust from this on each motor to maintain axial X,Y,Z/Pitch,Roll,Yaw balance
...
// Inc. auto-stablise even if RC roll, pitch or yaw input received
// Which motors to +/- thrust on depends on RC input
// Roll, Pitch, Yaw adjustment is based on adjusting thrust RC input by +/- % 
  ...
  // rc_channel_4 = yaw
   ...
   // Set ALL motors to thrust level dictated by rc_channel_1 input (manual altitude thrust)
...
// ***************************
  ...
  // Auto Stablise PID additions/correction for main RC Altitude input + Pitch, Roll, Yaw RC additions to RC Roll/Pitch/Yaw inputs
  thrustLF_PWM=rc_channel_1+PID_Output_Xaxis-PID_Output_Yaxis+PID_Output_Zaxis-rc_channel_2-rc_channel_3+rc_channel_4;
  thrustRF_PWM=rc_channel_1+PID_Output_Xaxis+PID_Output_Yaxis-PID_Output_Zaxis+rc_channel_2-rc_channel_3-rc_channel_4;
  thrustLA_PWM=rc_channel_1-PID_Output_Xaxis-PID_Output_Yaxis-PID_Output_Zaxis-rc_channel_2+rc_channel_3-rc_channel_4;
  thrustRA_PWM=rc_channel_1-PID_Output_Xaxis+PID_Output_Yaxis+PID_Output_Zaxis+rc_channel_2+rc_channel_3+rc_channel_4;
