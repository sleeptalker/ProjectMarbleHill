...
#define KP_QUAD_ROLL 1.75
#define KD_QUAD_ROLL 0.45
#define KI_QUAD_ROLL 0.5
#define KP_QUAD_PITCH 1.75
#define KD_QUAD_PITCH 0.45
#define KI_QUAD_PITCH 0.5
#define KP_QUAD_YAW 3.2  //2.6
#define KD_QUAD_YAW 0.8  //0.4
#define KI_QUAD_YAW 0.2  //0.15
...
#define Kp_ROLLPITCH 0.0125 // Pitch&Roll Proportional Gain
#define Ki_ROLLPITCH 0.000010 // Pitch&Roll Integrator Gain
#define Kp_YAW 3.0 // Yaw Porportional Gain  
...
float comando_rx_roll;        // comandos recibidos rx
float comando_rx_pitch;
float comando_rx_yaw;
...
void Attitude_control(){
  ...
  //roll_D = (err_roll-err_roll_ant)/G_Dt;    // Typical D Term...
  ...
  // PID control
  //control_roll = 0.8*err_roll + 0.2*roll_D + 0.5*roll_I; //0.9 25 // 1.2 40.0
  control_roll = KP_QUAD_ROLL*err_roll + KD_QUAD_ROLL*roll_D + KI_QUAD_ROLL*roll_I; //1.75 0.35 0.5 // 0.8 0.3 0.6
  ...
  //pitch_D = (err_pitch-err_pitch_ant)/G_Dt;  
  // To have a better (faster) response we can use the Gyro reading directly for the Derivative term...
  pitch_D = -ToDeg(Omega[1]);
 
  // PID control
  control_pitch = KP_QUAD_PITCH*err_pitch + KD_QUAD_PITCH*pitch_D + KI_QUAD_PITCH*pitch_I; // + 0.5*roll_I; // 1.2 40.0
  ...
  aux_debug = (err_yaw-err_yaw_ant)/G_Dt;
  // To have a better (faster) response we can use the Gyro reading directly for the Derivative term...
  yaw_D = -ToDeg(Omega[2]);
...
void setup(){
  ...
  Servo2 = 1500;
  Serial.println();
  Serial.println("JJ ArduIMU Quadcopter 1.15 PID");
  RxServoInput_ini();
  delay(1000);
 ...
 Neutro[1] = Neutro[1]/10;
 Neutro[2] = Neutro[2]/10;
 Neutro[3] = Neutro[3]/10;
 Neutro[4] = Neutro[4]/10;
 ...
 while (RxGetChannelPulseWidth(3)>(MIN_THROTTLE+40)){
...
void loop(){
  ...
  if((millis()-timer)>=14)   // 14ms => 70 Hz loop rate 
  {
    ...
    if (radio_status == 1){
      ...
      comando_rx_roll = (ch1-Neutro[1])/18.0;
      comando_rx_pitch = (ch2-Neutro[2])/18.0;
      aux_float = (ch4-Neutro[4])/400.0;
      ...
      K_aux = (float)(RxGetChannelPulseWidth(4)-Neutro[4])/100.0 + 0.6;
      if (K_aux < 0)
        K_aux = 0;
      ...
      if (ch3<MIN_THROTTLE)
        ch3 = MIN_THROTTLE;
      comando_rx_roll = 0;     // Stabilize to roll=0, pitch=0, yaw not important
      comando_rx_pitch = 0;
      Attitude_control();
      // Quadcopter mix
      Servo_Timer2_set(0,ch3 - control_roll + control_yaw);    // Right motor
    ...
    */
    ...
    if (data_update_event==0x03){          // New GPS info?
      data_update_event=0;
      
      if (gpsFix==0)
        GPS_Error = 0;
      //navigation_calc();
      // Altitude Navigation control (if we are in AP mode)
      //if (AP_mode==3)
      //  navigation_control_altitude();
      
      if (GPS_Error == 0){    
        Serial.print(",");
        Serial.print(long(lon*1000000));
        Serial.print(",");
        Serial.print(long(lat*1000000));
        Serial.print(",");
        Serial.print(int(alt_MSL*10));
        //Serial.print(",0");
        //Serial.print(GPS_hdop);
        //Serial.print(",");
        //Serial.print(int(ground_course));
        //Serial.print(",");
        //Serial.print(int(speed_3d*3.6));    // km/h
        //Serial.print(",");
        //Serial.print(int(course_to_dest));
        //Serial.print(",");
        //Serial.print(int(cmd_AP_roll));
        //Serial.print(",");
        //Serial.print(int(cmd_AP_pitch));
        //Serial.print(",");
